...
export function OrbitViewer() {
  ...
  useEffect(() => {
    ...
    setError("");
    fetch(assetUrl(record.trajectory), { signal: controller.signal }).then((response) => {
      ...
      if (!controller.signal.aborted) setError("The selected trajectory is missing or malformed.");
    });
    ...
    <main className="site-shell">
      ...
      <div className="content">
        ...
        <aside className={`system-drawer${isSystemDrawerOpen ? " is-open" : ""}`} aria-labelledby="system-heading">
          <div className="system-drawer-bar"><div><p className="eyebrow">Physical model</p><h2 id="system-heading">Egalitarian double pendulum</h2></div><button className="drawer-toggle" type="button" aria-expanded={isSystemDrawerOpen} aria-controls="system-drawer-content" onClick={() => setIsSystemDrawerOpen((value) => !value)}><span className="drawer-icon" aria-hidden="true" />{isSystemDrawerOpen ? "Minimize" : "Expand"}</button></div>
